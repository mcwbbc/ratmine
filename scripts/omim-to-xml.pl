...
while(<$IN>)
{
	...
	$omim_attr{primaryIdentifier} = $info[9];
	$omim_attr{title} = $info[7] . $info[8];
	my $status = getStatus($info[6]);
	$omim_attr{status} = $status if $status;
	$omim_attr{methods} = getMethods($info[10]);
	$omim_attr{dateEntered} = "$info[1]-$info[2]-$info[3]";
	$omim_attr{associatedDisorders} = getDisorders("$info[13]$info[14]$info[15]");
...
sub getDisorders
{
	...
	foreach my $do (split(/;/, $disorders))
	{
		if($do =~ /, (\d{6}) \((\d)\)/)
		{
			...
			$do_attr{primaryIdentifier} = $id;
			$do_attr{title} = $title;
			$do_attr{methods} = [$phMethod_item] if $phMethod_item;
			
			$disorder_items{$id} = $item_doc->add_item(OMIM => %do_attr) if $id;
			push @disorder_items, $disorder_items{$id} if $id;
		}
	}
	return \@disorder_items;
} #getDisorders
