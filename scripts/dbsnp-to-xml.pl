...
my $chr_items = RCM::addRatChromosomes($item_doc, $org_item);
...
sub processDbSNPFile
{
	...
	while(<$SNP>)
	{
		...
		if( $entry =~ m|<Rs [\d\D]+?</Rs>|) #grabs an Rs item
		{
			...
			my $maplocs = $xp->find('//Assembly[@groupLabel="'.$assembly.'"]/Component/MapLoc');
			...
			$snp_attr{consequenceTypes} = \@consequences;
